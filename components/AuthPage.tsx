...
export const AuthPage: React.FC<AuthPageProps> = ({ mode, onClose, onAuthSuccess, onSwitchMode }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (mode === 'signup') {
        ...
        // Add user to the global list for admin tracking
        try {
            const allUsersJSON = localStorage.getItem('geosick_all_users');
            const allUsers = allUsersJSON ? JSON.parse(allUsersJSON) : [];
            if (!allUsers.includes(nickname)) {
                allUsers.push(nickname);
                localStorage.setItem('geosick_all_users', JSON.stringify(allUsers));
            }
        } catch (e) {
            console.error("Failed to update all_users list", e);
        }
    ...
    const isAdmin = nickname === 'nikki' && password === '123456';

    if (mode === 'login') {
        addHistoryEvent(nickname, 'login', {});
    }
    
    onAuthSuccess({ nickname, isAdmin });
  };
